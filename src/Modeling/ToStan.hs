...
stanRender = \case
  Var x    -> x
  App (App (SCon "disj") x) (Pair y z) -> "mixture(" ++ show x ++ ", " ++ stanRender y ++ ", " ++ stanRender z ++ ")"
...
toStan = \case
  ...
  result   -> do
    pure (Model [("y", stanRender result)])
...
simplified = asTyped tau (betaDeltaNormal deltaRules . respond (lam x (normalL x)) knowPrior) discourse
